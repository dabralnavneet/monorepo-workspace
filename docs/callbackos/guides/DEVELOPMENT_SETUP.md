...
pnpm --filter @callbackos/web <script>
pnpm --filter @callbackos/server <script>
pnpm --filter @callbackos/database <script>

# Examples
pnpm --filter @callbackos/server test
pnpm --filter @callbackos/web build
pnpm --filter @callbackos/database db:studio
```
...
pnpm --filter @callbackos/web add axios
pnpm --filter @callbackos/server add @nestjs/config

# Add as dev dependency
pnpm --filter @callbackos/server add -D @types/multer

# Add to the root (shared dev tools only)
pnpm add -D prettier -w

# Add a workspace package as a dependency of another
pnpm --filter @callbackos/server add @callbackos/types@workspace:*
pnpm --filter @callbackos/web add @callbackos/types@workspace:*
```
...
3. Create a new project (name: `callbackos`)
4. Copy the connection string from the dashboard
5. Paste it into your `.env` file as `DATABASE_URL`

```env
DATABASE_URL="postgresql://user:password@ep-cool-forest-123456.us-east-2.aws.neon.tech/callbackos?sslmode=require"
```
...
│   ├── PROJECT_OVERVIEW.md         # What is CallbackOS? Business context
│   ├── architecture/
