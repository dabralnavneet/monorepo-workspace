...
CallbackOS uses **PostgreSQL** (hosted on Neon) with **Prisma ORM**. The schema is defined in:

```
packages/database/prisma/schema.prisma
```

Both `apps/web` and `apps/server` share the same Prisma client via the `@callbackos/database` package.
