...
HireFlow is a **Turborepo monorepo** with a clear separation between frontend, backend, and shared packages.
...
- **Usage**: Both `apps/web` and `apps/server` import from `@hireflow/database`
...
- **Import**: `import { User, InterviewStage } from '@hireflow/types'`
...
- **Import**: `import { calculateMatchScore } from '@hireflow/utils'`
